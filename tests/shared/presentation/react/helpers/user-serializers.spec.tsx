...
import {
  ...
  formatEnrollmentId,
  formatCpf
} from '@/shared/presentation/react/helpers/user-serializers'
...
describe('User Serializers', () => {
  describe('formatUserRole', () => {
    ...
    test('should return "Desconhecido" if role is falsy', () => {
      expect(formatUserRole(null as unknown as User['role'])).toBe(
        'Desconhecido'
      )
      expect(formatUserRole(undefined as unknown as User['role'])).toBe(
        'Desconhecido'
      )
    })

    test('should return the role itself if unknown', () => {
      expect(formatUserRole('CUSTOM_ROLE' as unknown as User['role'])).toBe(
        'CUSTOM_ROLE'
      )
    })
  })
  ...
  describe('formatUserStatus', () => {
    ...
    test('should return status itself if unknown', () => {
      expect(formatUserStatus('UNKNOWN' as unknown as User['status'])).toBe(
        'UNKNOWN'
      )
    })
  })
  ...
  describe('getUserStatusColor', () => {
    ...
    test('should return "danger" for BLOCKED', () => {
      expect(getUserStatusColor('BLOCKED')).toBe('danger')
    })

    test('should return "warning" for other statuses', () => {
      expect(getUserStatusColor('INACTIVE')).toBe('warning')
      expect(getUserStatusColor('UNKNOWN' as unknown as User['status'])).toBe(
        'warning'
      )
    })
  })
